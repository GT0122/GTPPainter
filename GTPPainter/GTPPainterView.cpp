...
void CGTPPainterView::OnDraw(CDC* pDC)
{
	...
	if(!m_bTitleChange && m_bDrawn)
	{
		...
		GetParent()->SetWindowTextW('*' + strTitle);

		m_bTitleChange = true;

		m_vdState.m_bEdited = true;
		
		GetParent()->PostMessage(GTPPainter_DRAWING, true, 0);
		GetParent()->PostMessageW(GTPPainter_DATA, NULL, (LPARAM)&m_vdState);
	}
...
void CGTPPainterView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if(nFlags & MK_MBUTTON)
	{
		...
		m_vdState.m_f64OffsetX = m_ptdViewZero.m_f64X;
		m_vdState.m_f64OffsetY = m_ptdViewZero.m_f64Y;

		GetParent()->SendMessageW(GTPPainter_DATA, NULL, (LPARAM)&m_vdState);
	}
...
BOOL CGTPPainterView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	if(zDelta > 0)
	{
		if(m_f64Scale < pow(10, 5))
		{
			...
			m_vdState.m_f64OffsetX = m_ptdViewZero.m_f64X;
			m_vdState.m_f64OffsetY = m_ptdViewZero.m_f64Y;
			m_vdState.m_f64Scale = m_f64Scale;
...
afx_msg LRESULT CGTPPainterView::OnSaving(WPARAM wParam, LPARAM lParam)
{
	...
	int8_t* pI8FL = nullptr;
	pI8FL = (int8_t*)"FourthLogic";
	FILE* pFSave = nullptr;
	...
	if(pFSave)
	{
		CalcCrc32(&u32Crc32val, pI8FL, sizeof(int8_t) * 11);
		fwrite(pI8FL, sizeof(int8_t), 11, pFSave);
		...
		GetParent()->PostMessage(GTPPainter_DRAWING, false, 0);

		m_vdState.m_bEdited = false;
		GetParent()->PostMessage(GTPPainter_DATA, NULL, (LPARAM)&m_vdState);

		if((int32_t)wParam == -1)
			GetParent()->PostMessage(GTPPainter_CLOSING, true, (bool)lParam);
	}
	else
	{
	...
	if(pFLoad)
	{
		...
		for(uint32_t i = 0; i < u32Total; i++)
		{
			...
			for(uint32_t j = 0; j < u32Line; j++)
			{
				...
				m_vctPtReals.push_back(CPointF64(f64X, f64Y, u32Thick, sColorTemp));
			}
...
BOOL CGTPPainterView::OnEraseBkgnd(CDC* pDC)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
...
void CGTPPainterView::OnShowWindow(BOOL bShow, UINT nStatus)
{
